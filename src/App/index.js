...
window.slice.router.beforeEach(async (to, from, next) => {
  const db = openDatabase();
  const langSetting = await db.get("settings", "settings");

  if (to.metadata?.requiresNoSettings) {
    if (langSetting !== undefined) {
      console.log("Settings found, redirecting to home");
      return next({ path: "/" });
    }
    return true;
  } else {
    if (langSetting === undefined) {
      console.log("No settings found, redirecting to start");
