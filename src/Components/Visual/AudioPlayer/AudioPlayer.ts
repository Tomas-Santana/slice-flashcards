  ...
  async getTemplate() {
    ...
    const timeDisplay = this.props.compact
      ? html`
          <div class="flex items-center gap-1">
            <span class="ap-current-time text-xs text-font-secondary"
              >0:00</span
            >
      ...
      : html`
          <div class="flex-1 flex items-center gap-2">
            <span class="ap-current-time text-xs text-font-secondary w-10"
              >0:00</span
            >
            <div
              class="ap-progress flex-1 h-1 bg-gray-200 rounded-full cursor-pointer relative"
            >
              <div
                class="ap-progress-bar h-full bg-primary rounded-full"
                style="width: 0%"
              ></div>
            </div>
            <span class="ap-duration text-xs text-font-secondary w-10"
              >0:00</span
            >
          </div>
        `;
    ...
    return html`
      <div class="audioplayer flex items-center gap-3 p-3 bg-white rounded-md ">
        <audio
          class="ap-audio hidden"
          src="${this.props.audioUrl}"
          preload="metadata"
        ></audio>

        <div class="flex items-center gap-1">
          ${this.$playPauseBtn} ${this.$restartBtn}
        </div>
